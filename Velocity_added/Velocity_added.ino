...
double kib=0;
double kdb=0;
PID_control ctrl(kpa, kia, kda, 0, 255, sigma, dt/1e6);
PID_control ctrlb(kpb, kib, kdb, 0, 255, sigma, dt/1e6);
    ...
    if (lightsOn) {
      if(deltatime>=0 && deltatime < 10000){ //run until 10 seconds max
        ...
        { 
          ...
          //angular position control that outputs a position error for inner loop to match
          ...
          derivative=diff.differentiate(angle/16384.0);
          float u_vel= ctrl.pid(u_pos, derivative);
