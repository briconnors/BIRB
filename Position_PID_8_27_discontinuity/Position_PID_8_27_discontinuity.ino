...
//motor power setup
float frequency = 4; //input for position control reference angle
...
void loop() {
  ...
  if (resetFlag){
      ...
      pwra=constrain(pwra,0,150); //limit so it doesnt blow motors
    ...
    if (lightsOn) {
      if(deltatime>=0 && deltatime < 10000000){ //run until 10 seconds max
        ...
        { 
          ...
          float ref = fmod((360.0*cur/1000000.0),360);//sin(2*3.14159265359*frequency*cur/1000000.0); //reference position outputs value btwn -1 and 1
